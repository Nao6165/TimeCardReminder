...
## 使用方法
### 簡単な使用手順
1. ReleasesからTimeCardReminder_vx.x.x.zipをダウンロードして解凍。
2. TimeCardReminder.exeを実行して当アプリを起動。(タスクトレイに常駐します)
3. タスクトレイのアイコンを右クリックし、**設定**を選択。(MainWindowが起動します)
4. 通知したい**メッセージ(必須)**と**時刻(必須)**、**実行したいファイル(オプション)** を設定
5. **追加ボタン**を押下
6. チェックボックスにチェックが入っていることを確認した上で**設定ボタン**を押下
